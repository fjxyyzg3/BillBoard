...
- Windows 未登录自启动：生产主机使用计划任务 `BillBoard Production Startup` 调用 `ops/windows/start-production.ps1 -StayAttached`，注册脚本是 `ops/windows/register-production-startup.ps1`；本地未跟踪的 `.env.production.local` 必须包含真实 `AUTH_SECRET`，日志在 `tmp/logs/production-startup.log`。`LastTaskResult=267009` 表示该常驻任务正在运行。
