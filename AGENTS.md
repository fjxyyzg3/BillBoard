...
- 导入约束：随手记导入入口在 `/records/import`；导入记录使用 `source`、`sourceFingerprint`、`sourceImportedAt` 保存来源元数据；导入不新增分类，映射按 `交易类型 + 一级分类 + 二级分类` 聚合并由整个家庭共享；大草稿确认必须保持批量查询、批量写入和足够的 Prisma transaction timeout，避免逐行操作导致交互式事务超时。
...
- Windows 正式服务排障：正常路径仍是 `podman-compose.yml` 的 `web` 服务；若 Windows 上临时绕过 compose build、手动运行 `node .next/standalone/server.js`，必须先将 `.next/static` 复制到 `.next/standalone/.next/static`，否则 `/_next/static/chunks/*.js` 会 404，登录表单会退化为 `/login?email=...&password=...` 的普通 GET 提交。详细步骤见 `docs/runbooks/home-network-checklist.md`。
- Windows 未登录自启动：生产主机使用计划任务 `BillBoard Production Startup` 调用 `ops/windows/start-production.ps1 -StayAttached`，注册脚本是 `ops/windows/register-production-startup.ps1`；本地未跟踪的 `.env.production.local` 必须包含真实 `AUTH_SECRET`，域名入口还必须包含 `APP_DOMAIN`。该脚本会启动 fallback `web`、Caddy `billboard_proxy_1`、`0.0.0.0:3000 -> web:3000` 隧道，以及 `127.0.0.1:8080/8443 -> Caddy:80/443` 隧道；frpc 的公网 `80/443` 映射应指向本机 `8080/8443`。日志在 `tmp/logs/production-startup.log`。`LastTaskResult=267009` 表示该常驻任务正在运行。
- 启动服务测试：开发服默认使用 `2500`；手动启动 Next dev 时如果不通过 Playwright 加载 env，必须提供 `AUTH_SECRET`（开发可用 `replace-me`）和 `DATABASE_URL`，否则 Auth.js 会报 `MissingSecret` 或连接错误数据库；正式服务默认通过生产 compose 暴露 `http://<host>:3000`，同时保留 `proxy` 的 `80/443 -> web:3000` 入口。当用户要求启动开发服务进行测试时，按访问方式选择 host。局域网直连优先绑定当时机器的局域网 IP；若 frpc 或隧道配置指向 `127.0.0.1:2500`，必须绑定 `0.0.0.0` 或确认 `127.0.0.1:2500` 正在监听。通过外网 IP 或域名访问 Next dev 时，必须确认 `next.config.ts` 的 `allowedDevOrigins` 包含该 host，否则客户端 JS/HMR 可能被拦截，登录表单会退化为普通 GET 提交。
